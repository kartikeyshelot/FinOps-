...
from rds_mysql_sa_prices import RDS_MYSQL_SA_HOURLY as _SEED_RDS
from ec2_ondemand_public import EC2_ONDEMAND_BY_REGION_OS as _SEED_EC2, EC2_ONDEMAND_PUBLIC_AS_OF

logger = logging.getLogger(__name__)
CACHE_TTL_DAYS = 7

# ── Load generated pricing (written by scripts/fetch_pricing.py) ─────────────
# Falls back to seed data when the generated file is absent (e.g. fresh clone).

_GENERATED: dict | None = None
_GENERATED_AS_OF: str = EC2_ONDEMAND_PUBLIC_AS_OF  # will be overwritten if generated file exists
try:
    from generated_pricing import GENERATED_PRICING, GENERATED_AS_OF  # type: ignore[import]
    _GENERATED = GENERATED_PRICING
    _GENERATED_AS_OF = GENERATED_AS_OF
    logger.info("Loaded pricing from generated_pricing.py (fetched %s)", GENERATED_AS_OF)
except ImportError:
    logger.info("generated_pricing.py not found — using bundled seed data. Run: python scripts/fetch_pricing.py")


def _build_ec2_table() -> dict[str, dict[str, dict[str, float]]]:
    """Build region → os → instance_type → hourly price from generated or seed data."""
    if _GENERATED and _GENERATED.get("ec2"):
        table: dict[str, dict[str, dict[str, float]]] = {}
        for region, instances in _GENERATED["ec2"].items():
            if isinstance(instances, dict) and "error" not in instances:
                os_map: dict[str, dict[str, float]] = {}
                for inst, info in instances.items():
                    for os_key, price in (info.get("prices") or {}).items():
                        os_map.setdefault(os_key, {})[inst] = price
                table[region] = os_map
        # Merge seed regions that generated data didn't cover
        for region, bundle in _SEED_EC2.items():
            if region not in table:
                table[region] = bundle
        return table
    return dict(_SEED_EC2)


def _build_rds_table() -> dict[str, dict[str, float]]:
    """Build region → db_class → mysql hourly price from generated or seed data."""
    if _GENERATED and _GENERATED.get("rds"):
        table: dict[str, dict[str, float]] = {}
        for region, payload in _GENERATED["rds"].items():
            if isinstance(payload, dict) and "error" not in payload:
                instances = payload.get("instances", {})
                region_prices: dict[str, float] = {}
                for db_class, info in instances.items():
                    mysql_price = (info.get("prices") or {}).get("mysql")
                    if mysql_price is not None:
                        region_prices[db_class] = mysql_price
                if region_prices:
                    table[region] = region_prices
        # Fill any seed regions not covered by generated data
        for region, prices in _SEED_RDS.items():
            if region not in table:
                table[region] = prices
        return table
    return dict(_SEED_RDS)


EC2_ONDEMAND_BY_REGION_OS: dict[str, dict[str, dict[str, float]]] = _build_ec2_table()
RDS_MYSQL_SA_HOURLY: dict[str, dict[str, float]] = _build_rds_table()
...
_using_generated = _GENERATED is not None
CACHE_METADATA: dict = {
    'last_updated': _manifest_as_datetime(_GENERATED_AS_OF),
    'source': (
        'AWS Bulk Pricing API (on-demand, all services)'
        if _using_generated
        else 'AWS EC2 On-Demand — public meteredUnitMaps (per-OS SKU; same feed as aws.amazon.com)'
    ),
    'version': '3.0' if _using_generated else '2.0',
}
PRICING_SOURCE_LABEL: str = (
    'AWS Bulk Pricing API — fetched on-demand data'
    if _using_generated
    else 'Bundled AWS EC2 meteredUnitMaps (on-demand)'
)
...
# Extend supported regions when generated data is available
_GENERATED_REGION_NAMES: dict[str, str] = (
    (_GENERATED or {}).get("meta", {}).get("regions", {})
)
_BASE_SUPPORTED: list[tuple[str, str]] = [
    ('eu-west-1', 'EU (Ireland)'),
    ('us-east-1', 'US East (N. Virginia)'),
    ('ap-south-1', 'Asia Pacific (Mumbai)'),
    ('eu-central-1', 'EU (Frankfurt)'),
]
_BASE_IDS: frozenset[str] = frozenset(r for r, _ in _BASE_SUPPORTED)
_EXTRA_SUPPORTED: list[tuple[str, str]] = [
    (rid, label)
    for rid, label in _GENERATED_REGION_NAMES.items()
    if rid not in _BASE_IDS and rid in EC2_ONDEMAND_BY_REGION_OS
]
SUPPORTED_REGIONS: list[tuple[str, str]] = _BASE_SUPPORTED + sorted(_EXTRA_SUPPORTED)
...
PRICE_CACHE: dict[str, dict[str, float]] = {r: EC2_ONDEMAND_BY_REGION_OS[r].get('linux', {}).copy() for r in EC2_ONDEMAND_BY_REGION_OS}
...
def format_pricing_snapshot_line(pricing_region_id: str) -> str:
    ...
    return f'Pricing Snapshot: {rid} | Source: {PRICING_SOURCE_LABEL} | EC2 as of: {as_of} (manifest {_GENERATED_AS_OF})'
