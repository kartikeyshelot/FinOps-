...
def process(df: pd.DataFrame, binding: ColumnBinding, region: str=DEFAULT_REGION, service: ServiceMode='both', cpu_filter: CPUFilterMode='both') -> pd.DataFrame:
    ...
    for i in range(n):
        ...
        try:
            ...
            rec_key = (inst, backend, cpu, row_region)
            if rec_key not in rec_cache:
                rec = (
                    get_rds_recommendations(inst, region=row_region, cpu_filter=cpu)
                    if backend == 'rds'
                    else get_recommendations(inst, region=row_region, cpu_filter=cpu)
                )
