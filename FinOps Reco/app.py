...
with st.container(border=True):
    ...
    with cpu_col:
        st.session_state['cpu_filter'] = st.selectbox(
            'cpu', ['both', 'default', 'intel', 'amd', 'graviton'],
            format_func=lambda x: {'both': 'Both', 'default': 'Default', 'intel': 'Intel', 'amd': 'AMD', 'graviton': 'Graviton'}[x],
...
if df_out is not None:
    with st.container(border=True):
        ...
        with f2:
            ...
            _cpu_label = {'both': 'Both', 'default': 'Default', 'intel': 'Intel', 'amd': 'AMD', 'graviton': 'Graviton'}
            st.write(_cpu_label.get(str(st.session_state.get('cpu_filter', 'both')), 'Both'))
        with f3:
